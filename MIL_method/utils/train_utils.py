...
def group_topk(probs, topk, k, thresholde=0.5, pos_label=1):
    # 该方法实现通过基于前topk个的patch的概率得到的patch标签,
    # 将1类数量patch占全部patch数量的比例作为该slide最终的预测标签
    select_probs = np.array([probs[x, :] for x in topk])
    prob_result = []

    for j in range(0, select_probs.shape[0], k):
        result = np.sum(select_probs[j:j + k, pos_label] > thresholde) / k
        prob_result.append(result)
